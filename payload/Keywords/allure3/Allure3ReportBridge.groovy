 ...
 *    versions. Finer, in-test detail is opt-in via Allure3Keywords.step(),
 ...
 *    queuePendingSteps()).
 */
class Allure3ReportBridge {

    private static final KeywordLogger logger = KeywordLogger.getInstance(Allure3ReportBridge.class)
    ...
    static void startSuite(TestSuiteContext testSuiteContext) {
        try {
            ...
            File resultsDir = Allure3Config.getResultsDir()
            resultsDir.mkdirs()
            ...
            withRunLock(resultsDir) {
                ...
                if (!continuingRun) {
                    if (Allure3Config.cleanResultsBeforeRun()) {
                    ...
                    new File(resultsDir, '.allure3-collection-progress.txt').delete()
                }
            ...
            logger.logInfo("[Allure3] Reporting enabled. Results directory: ${resultsDir.absolutePath}")
        } catch (Throwable t) {
            logger.logWarning("[Allure3] Failed to initialize Allure reporting: ${t}")
        }
    }
    ...
    static void finishSuite(TestSuiteContext testSuiteContext) {
        try {
            ...
            File resultsDir = Allure3Config.getResultsDir()
            logger.logInfo("[Allure3] Suite finished. Results: ${resultsDir.absolutePath}")
     ...
     */
    /**
     * Runs the Allure 3 CLI synchronously so a finished HTML report is
     * already sitting on disk the moment the suite finishes.
     *
     * Allure 3 differs from Allure 2 here in three ways that matter:
     *  - "--single-file" moved off "generate" and onto the report plugin,
     *    so single-file mode runs "allure awesome --single-file" rather
     *    than "allure generate --single-file".
     *  - There is no "--clean". Nothing needs one: every run generates
     *    into its own freshly-named staging directory, and the swap into
     *    place below is what publishes it.
     *  - History is no longer a "history/" folder copied into the results
     *    directory before generating. Allure 3 keeps it in a single JSONL
     *    file passed as "--history-path", which it appends to on each
     *    generation. That file lives outside the report, so single-file
     *    reports now carry Trend and Retries forward too - something
     *    Allure 2's embedded-history model made impossible.
     *
     * Deliberately generate-only, not serve/open: this must stay safe to
     * run unattended in CI (no server, no browser, exits on its own).
     * Never throws - if the CLI can't be found, this logs one clear
     * warning and the suite result is unaffected either way.
     */
    private static void generateHtmlReport(File resultsDir, String suiteName) {
        if (!Allure3Config.autoGenerateReport()) {
            return
        }
        File reportBaseDir = Allure3Config.getReportDir()
        File stagingDir = new File(reportBaseDir, ".staging-${System.nanoTime()}")
        List<String> cli = resolveAllureCli()
        try {
            reportBaseDir.mkdirs()
            boolean singleFile = Allure3Config.singleFileReport()

            logger.logInfo('[Allure3] Generating HTML report - this can take a few seconds, there is no separate progress indicator for it.')
            if (!runAllureGenerate(cli, resultsDir, stagingDir, singleFile, suiteName)) {
                return
            }

            // Only the swap-into-place + dedup + marker update is locked -
            // not the generate call above, which already writes into this
            // call's own uniquely-named staging dir and is the slow part,
            // so suites running in parallel still generate concurrently.
            withRunLock(resultsDir) {
                ...
                // If an earlier Test Suite in this same overall run already
                // produced a report, remove it - a multi-suite run should
                // end up with exactly one report, not one per sub-suite.
                Map marker = readRunMarker(resultsDir)
                ...
                logger.logInfo("[Allure3] HTML report ready: ${finalTarget.absolutePath}")
            }
        } catch (Throwable t) {
            logger.logWarning("[Allure3] Could not auto-generate the HTML report using '${cli.join(' ')}' - reinstall the bridge " +
                '(node <bridge>/bin/cli.js install <project>), point allure3.cli.path / ALLURE3_CLI_PATH at an Allure 3 CLI, ' +
                "or set allure3.auto.generate.report=false. (${t.getMessage()})")
        } finally {
    ...
    /**
     * Builds and runs the actual Allure 3 command line.
     *
     * Single-file mode goes through the "awesome" plugin command, because
     * that is where Allure 3 exposes --single-file. Multi-file mode uses
     * plain "generate", whose default plugin is awesome anyway, so both
     * modes produce the same report - only the packaging differs.
     *
     * A project-level allurerc file, when one is present, is passed with
     * -c and takes precedence for everything it declares (environments,
     * categories, quality gate). The flags below stay as the baseline for
     * projects that never customize it.
     */
    private static boolean runAllureGenerate(List<String> cli, File resultsDir, File outputDir, boolean singleFile, String suiteName) {
        List<String> command = new ArrayList<String>(cli)
        command.add(singleFile ? 'awesome' : 'generate')
        command.add(resultsDir.absolutePath)
        command.add('--output')
        command.add(outputDir.absolutePath)
        command.add('--report-name')
        command.add(suiteName ?: 'Katalon Test Run')
        if (singleFile) {
            command.add('--single-file')
        }

        // Only "awesome" accepts --history-path on the command line;
        // "generate" reads historyPath from the config file instead, which
        // is why the installer writes one.
        File history = Allure3Config.getHistoryFile()
        if (history != null && singleFile) {
            history.parentFile?.mkdirs()
            command.add('--history-path')
            command.add(history.absolutePath)
        }

        File config = Allure3Config.getConfigFile()
        if (config != null && config.isFile()) {
            command.add('-c')
            command.add(config.absolutePath)
        }
    ...
    /** Cached for the lifetime of this process - resolution touches the filesystem, and does not need to repeat per suite. */
    private static volatile List<String> resolvedAllureCli = null

    /**
     * Resolves how to invoke the Allure 3 CLI, as a command prefix such as
     * ["C:\\Program Files\\nodejs\\node.exe", "...\\allure\\dist\\cli.js"].
     *
     * Allure 3 is a Node program, not the Java launcher Allure 2 shipped,
     * and this bridge pins it as its own npm dependency rather than asking
     * the user to install one globally. That removes the entire class of
     * PATH problems the Allure 2 bridge had to work around: a Katalon
     * Studio IDE launched from Finder/Dock/Start Menu never sources the
     * user's login shell profile, so volta/nvm/homebrew installs were
     * invisible to it even when "which allure" worked in a terminal.
     * Invoking node directly on the CLI's own entry point also sidesteps
     * Windows' inability to launch a .cmd shim from ProcessBuilder, so no
     * "cmd /c" wrapper is needed for the pinned case.
     *
     * Order: explicit config, then whatever the installer recorded for
     * this project, then a copy vendored into the project, then the
     * project's own node_modules, and finally a bare "allure" on PATH for
     * CI images that already provide one.
     */
    private static List<String> resolveAllureCli() {
        if (resolvedAllureCli == null) {
            synchronized (INTRA_JVM_LOCK) {
                if (resolvedAllureCli == null) {
                    resolvedAllureCli = findAllureCli()
                }
            }
        }
        return resolvedAllureCli
    }

    private static List<String> findAllureCli() {
        String configured = Allure3Config.getCliPath()
        if (configured) {
            List<String> fromConfig = cliCommandFor(new File(configured))
            if (fromConfig) {
                logger.logInfo("[Allure3] Using configured allure3.cli.path: ${configured}")
                return fromConfig
            }
            logger.logWarning("[Allure3] allure3.cli.path is set to '${configured}' but that is not a usable Allure 3 CLI - auto-detecting instead.")
        }

        File projectDir = new File(RunConfiguration.getProjectDir())

        // What the installer recorded, including the node executable it
        // resolved the CLI with - see bin/cli.js writeCliDescriptor().
        List<String> recorded = cliFromDescriptor(new File(projectDir, '.allure3-bridge/cli.json'))
        if (recorded) {
            logger.logInfo("[Allure3] Using the Allure 3 CLI recorded at install time: ${recorded.last()}")
            return recorded
        }

        // "allure/cli.js", not "allure/dist/cli.js": the package's bin
        // entry point sits at its root and only re-exports dist/index.js.
        List<String> localCandidates = [
            '.allure3-bridge/cli/node_modules/allure/cli.js',
            'node_modules/allure/cli.js',
        ]
        for (String candidate : localCandidates) {
            List<String> local = cliCommandFor(new File(projectDir, candidate))
            if (local) {
                logger.logInfo("[Allure3] Using the Allure 3 CLI found in the project: ${candidate}")
                return local
            }
        }

        logger.logInfo('[Allure3] No pinned Allure 3 CLI found - falling back to "allure" on PATH.')
        boolean isWindows = System.getProperty('os.name', '').toLowerCase().contains('win')
        return isWindows ? ['cmd', '/c', 'allure'] : ['allure']
    }

    /** Reads the {node, cli} pair the installer recorded, ignoring it if either path has since gone away. */
    private static List<String> cliFromDescriptor(File descriptor) {
        try {
            if (!descriptor.isFile()) {
                return null
            }
            JsonObject json = JsonParser.parseString(descriptor.text).getAsJsonObject()
            String cliPath = json.has('cli') ? json.get('cli').getAsString() : null
            String nodePath = json.has('node') ? json.get('node').getAsString() : null
            if (!cliPath || !new File(cliPath).isFile()) {
                return null
            }
            if (nodePath && new File(nodePath).isFile()) {
                return [nodePath, cliPath]
            }
            return cliCommandFor(new File(cliPath))
        } catch (Throwable ignored) {
            return null
        }
    }
    ...
    /**
     * Turns a path into a runnable command prefix. A .js entry point needs
     * a node executable in front of it; anything else is assumed to be
     * directly executable.
     */
    private static List<String> cliCommandFor(File file) {
        if (file == null || !file.isFile()) {
            return null
        }
        if (file.name.endsWith('.js')) {
            String node = resolveNodeExecutable()
            return node ? [node, file.absolutePath] : null
        }
        return isExecutableFile(file) ? [file.absolutePath] : null
    }
    ...
    private static boolean isExecutableFile(File f) {
        try {
            ...
            return f.canExecute() || lower.endsWith('.bat') || lower.endsWith('.cmd') || lower.endsWith('.exe')
        } catch (Throwable ignored) {
            return false
        }
    }
    ...
    /**
     * Finds a node executable to run the CLI with. The installer normally
     * records the exact one it used, so this is only reached when that
     * record is missing or stale; the locations below are the standard
     * install paths, with a bare "node" as the last resort.
     */
    private static String resolveNodeExecutable() {
        String configured = Allure3Config.getNodePath()
        ...
        if (isWindows) {
            String programFiles = System.getenv('ProgramFiles') ?: 'C:\\Program Files'
            candidates = [
                "${programFiles}\\nodejs\\node.exe".toString(),
     ...
     * fresh below anyway), and never touches anything else a
     ...
     * allure-results directory, turn this off (allure3.clean.results
    ...
    private static Map readRunMarker(File resultsDir) {
        File markerFile = new File(resultsDir, '.allure3-run-marker.txt')
    ...
    private static void writeRunMarker(File resultsDir, String runDir, String reportPath) {
        try {
            new File(resultsDir, '.allure3-run-marker.txt').text = "${runDir ?: ''}\n${reportPath ?: ''}\n"
        } catch (Throwable ignored) { }
    }
    ...
    private static Object withRunLock(File resultsDir, Closure action) {
        synchronized (INTRA_JVM_LOCK) {
            RandomAccessFile raf = new RandomAccessFile(new File(resultsDir, '.allure3-run.lock'), 'rw')
            try {
    ...
    private static boolean shouldGenerateReportNow(File resultsDir) {
        ...
        return withRunLock(resultsDir) {
            File progressFile = new File(resultsDir, '.allure3-collection-progress.txt')
    ...
    /**
     * Schedules an Allure result for a test case that is about to run.
     *
     * The identity fields here are where Allure 3 differs most sharply
     * from Allure 2, and getting them wrong is not cosmetic - it silently
     * turns a failing run green.
     *
     * Allure 2 honoured whatever historyId a result declared, so the
     * Allure 2 bridge disambiguated runs of one reusable test case by
     * hashing the suite into historyId itself. Allure 3 ignores a
     * declared historyId entirely and recomputes its own as
     * "testCase.id + hash(parameters)", where testCase.id comes from the
     * ALLURE_ID label, else testCaseId, else fullName - in that order, so
     * a bridge-written fullName never even gets consulted while
     * testCaseId is set. Retries are then grouped by
     * md5(testCaseId : parametersHash : environmentId), and only the
     * latest attempt in a group is shown.
     *
     * Katalon reuses one test case across many suites and browsers, all
     * sharing a testCaseId, so under Allure 3 every one of those
     * executions collapses into a single test with the rest hidden as
     * retries - a failure in any run but the last disappears, and the
     * report reads "passed".
     *
     * Two fields fix it, and both are needed:
     *  - the Suite parameter below feeds parametersHash, separating the
     *    same test case run from different suites;
     *  - the browser label added in finishTestCase() is what an
     *    allurerc environments matcher keys on, separating the same test
     *    case run against different browsers.
     *
     * historyId is still written, both because the Allure 2 reader
     * carries it through for trend continuity and because it costs
    ...
    static void startTestCase(TestCaseContext testCaseContext) {
        try {
            ...
            String qualifiedSuite = "${suiteName}${suiteLabelSuffix(suiteName)}".toString()
            ...
            result.setLabels([
                ResultsUtils.createSuiteLabel(qualifiedSuite),
            ...
            if (Allure3Config.separateSuitesAsParameters()) {
                result.setParameters([ResultsUtils.createParameter('Suite', qualifiedSuite)])
            }

            Allure.getLifecycle().scheduleTestCase(uuid, result)
            Allure.getLifecycle().startTestCase(uuid)
        } catch (Throwable t) {
            logger.logWarning("[Allure3] Failed to start test case reporting: ${t}")
        }
    }
    ...
    static void finishTestCase(TestCaseContext testCaseContext) {
        try {
            ...
            boolean shouldScreenshot = Allure3Config.attachScreenshotAlways() ||
                (Allure3Config.attachScreenshotOnFailure() && status != Status.PASSED)
            if (shouldScreenshot) {
            ...
            Allure.getLifecycle().updateTestCase(uuid, { TestResult tr ->
                ...
                if (activeBrowser) {
                    // A "browser" label, not a browser suffix folded into
                    // the suite name. Allure 2 had nowhere better to put
                    // it, so the Allure 2 bridge appended it to the visible
                    // suite label; Allure 3 has a first-class environments
                    // concept, and an environments matcher keys on a label,
                    // not on a suite name it would have to parse a
                    // parenthesised suffix back out of. The installer's
                    // generated allurerc matches exactly this label - see
                    // writeAllurercConfig() in bin/cli.js.
                    tr.getLabels()?.add(ResultsUtils.createLabel('browser', activeBrowser))

                    // The same browser again as a parameter, which is not
                    // redundant. An environment separates tests in the
                    // report body, but Allure 3 computes historyId as
                    // "testCase.id + hash(parameters)" with no environment
                    // component, so two browsers of one suite+test case
                    // otherwise share a single history entry: the newest
                    // run overwrites the other, and a browser-specific
                    // failure disappears from Trend while showing up as a
                    // spurious "flaky" flag. Adding the browser to the
                    // parameters gives each one its own history line.
                    if (Allure3Config.separateBrowsersAsParameters()) {
                        List params = tr.getParameters() ?: []
                        if (!params.any { it.getName() == 'Browser' }) {
                            params.add(ResultsUtils.createParameter('Browser', activeBrowser))
            ...
            if (activeBrowser) {
                recordActiveBrowserInEnvironment(Allure3Config.getResultsDir(), activeBrowser)
            }
            ...
            logger.logWarning("[Allure3] Failed to finish test case reporting: ${t}")
        }
    }
    ...
    private static void queuePendingSteps(String uuid, String testCaseId) {
        try {
            ...
            File resultsDir = Allure3Config.getResultsDir()
            withRunLock(resultsDir) {
                new File(resultsDir, '.allure3-pending-steps.txt').append("${uuid}|${testCaseId}|${logFolder}\n")
            }
        } catch (Throwable ignored) { }
    }
    ...
    private static void processPendingSteps(File resultsDir, boolean isFinalSuite) {
        List<String> claimedLines = withRunLock(resultsDir) {
            File pendingFile = new File(resultsDir, '.allure3-pending-steps.txt')
        ...
        claimedLines.each { line ->
            ...
            try {
                diag << "[Allure3][step-diag] ${new Date()} uuid=${uuid} testCaseId=${testCaseId}\n"
        ...
        if (isFinalSuite) {
            ...
            withRunLock(resultsDir) {
                new File(resultsDir, '.allure3-pending-steps.txt').append(stillPending.collect { "${it}\n" }.join(''))
            }
        }
    }
    ...
    private static void retryPendingStepsSynchronously(File resultsDir, List<String> pendingLines) {
        StringBuilder diag = new StringBuilder()
        try {
            diag << "[Allure3][step-diag] ${new Date()} synchronous final-suite retry starting for ${pendingLines.size()} entries\n"
    ...
    /**
     * This library's own listener actions (Allure3ReportBridge.*,
     * Allure3Keywords.*) are filtered out; every other TestStepLogRecord
    ...
    private static boolean isInternalStepName(String stepName) {
        return stepName != null && (stepName.contains('Allure3ReportBridge.') || stepName.contains('Allure3Keywords.'))
    }

    /** Exposed for Allure3Keywords.attachScreenshot() so tests can capture ad-hoc evidence too. */
    ...
    private static String resolveSuiteName(TestSuiteContext testSuiteContext) {
        ...
        String diagText = """[Allure3][diag] run at ${new Date()}
...
"""
        ...
        try {
            new File(RunConfiguration.getProjectDir(), 'allure3-diag.txt').text = diagText
        } catch (Throwable ignored) { }
    ...
    private static void mergeIntoEnvironmentProperties(File resultsDir, Properties fresh) {
        ...
        withRunLock(resultsDir) {
            ...
            envFile.withOutputStream { out ->
                merged.store(out, 'Generated by Allure3TestListener - do not edit by hand')
            }
        }
    }
    ...
    private static void writeCategoriesJson(File resultsDir) {
        File src = Allure3Config.getCategoriesFile()
        if (src.exists()) {
